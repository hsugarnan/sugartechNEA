...
function AddSAFEReport() {
  ...
  const imagesListRef = ref(storage, "SAFE/");//imageListRef refers to where the blobs are being stored
  const uploadFile = () => {//a function to upload the file
    if (imageUpload == null) return;//if no image return nothing
    const imageRef = ref(storage, `SAFE/${imageUpload.name + v4()}`);
  ...
  return (
    <div className="App">
     <h1 className="title">Upload Safe Reports</h1>
      <input
        type="file"
        onChange={(event) => {
      ...
      {/* {imageUrls.map((url) => {
       ...
       { submit ? <div className = "buttonWrapper"><p className="alert1" >Upload Complete</p> <Link to="/AddEODReport">Log EOD Reports</Link> </div> : <p className="smallFont">SugarTech 2023©</p>  }
    </div>
  );
}

export default AddSAFEReport;